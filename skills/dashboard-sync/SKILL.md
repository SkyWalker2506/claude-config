...
- **Not-done:** `project = <PROJE_ANAHTARI> AND status NOT IN ("Done") ORDER BY status ASC, priority DESC` — fields: summary, status, priority, labels — max 50
- **Done:** `project = <PROJE_ANAHTARI> AND status = "Done" ORDER BY updated DESC` — fields: summary, status, priority — max 10
