# Skills Inventory
Auto-generated from global/skills/. Total: 54 skills.

## Slash Commands

| Skill | Trigger | Description |
|-------|---------|-------------|
| add-mcp | /add-mcp | Add optional MCP servers after initial install |
| admin-login | /admin-login | GitHub authentication and account management |
| agent-browser | /agent-browser | Browser automation CLI for AI agents |
| agent-refine | /agent-refine | Agent sistemini proje bazli refine et — eksik agent tespit, generate, update |
| agent-refine-knowledge | /agent-refine-knowledge | Agent'in AGENT.md ve knowledge dosyalarini opus ile review et |
| agent-setup | /agent-setup | Agent'i yeni Knowledge-First yapiya cevir — dizin olustur, AGENT.md olustur |
| agent-sharpen | /agent-sharpen | Agent'in bilgi dagarcigini keskinlestir — web'den ve kaynaklardan bilgi topla |
| agents | /agents | Tum agent'lari gruplu listele — aktif/pool, kategori, model, capabilities |
| audit | /audit | Proje kodu taraması — güvenlik, maliyet, performans, gereksiz dosya, API kontrol |
...
| forge | /forge | Tam otonom gelistirme dongusu — analysis, sprint, parallel tasks, PR/review |
| forge-analysis | /forge-analysis | Forge sonrası verimlilik analizi — run metriklerini ölç, pattern analiz |
| free-models | /free-models | Ücretsiz modelleri tara (Groq + HuggingFace), karşılaştır, raporla |
| hi-test | /hi-test | Tüm free/local modellere selam testi yap, skorları güncelle, durumu raporla |
...
| persona | /persona | Kisilik yonetimi — listele, degistir, olustur, duzenle |
| plugin-update | /plugin-update | Kurulu plugin'leri güncelle |
| prd | /prd | Generate a Product Requirements Document (PRD) for a new feature |
| project-analysis | /project-analysis | Proje analizi — güvenlik, maliyet, performans, teknik borc |
| project-index | /project-index | Proje index'i yönet — init, show, list, update |
| projects | /projects | Proje listesi ve yonetimi |
| push-all | /push-all | Tum projelerdeki commitlenmis degisiklikleri push et |
...
| yolo | /yolo | Tam otonom gorev calistirici — soru sormadan gidebildigi yere kadar gider |
| yolo-log | /yolo-log | Son /yolo calistirmasinin ne yaptigini goster |
